use crate::{
    actor_system::Ctx, models::{RSState, UIMode, UserInput, UserAction, InputEvent, PageType},
...
fn handle_conflicting_bindings(actions: &mut Vec<UserAction>, state: &RSState) {
    ...
    if actions.contains(&UserAction::Confirm) && actions.contains(&UserAction::OpenContextMenu(None)) {
